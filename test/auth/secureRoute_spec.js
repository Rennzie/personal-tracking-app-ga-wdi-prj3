...
const badUserIdToken = jwt.sign({sub: '5b7a98cda80abb34fef6dc15'}, secret, {expiresIn: '1hr'});
...
describe('secureRoute via POST /api/events', () => {
  ...
  it('should return a 401 and a User Not Found  message if there is no user in the db', done => {
    api.post('/api/events')
  ...
  xit('should return true if the token is verified', done => {
    api.post('/api/events')
      ...
      .end((err, res) => {
        console.log('the data is ===> ', res.body);
        done();
      });
  });
});
